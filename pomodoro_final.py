...
timer=None
clicked_start=False
clicked_short_pause=False
clicked_long_pause=False
...
def start_funct():
    global clicked_start
    global clicked_short_pause
    global clicked_long_pause
    clicked_start=True
    clicked_short_pause=False
    clicked_long_pause=False
    if count_minutes<minutes:
...
def pause_timer():
    ...
    if clicked_start==True:
        start_button["state"] = "active"
    elif clicked_short_pause==True:
        short_break_button["state"] = "active"
    elif clicked_long_pause==True:
        long_break_button["state"] = "active"
    window.after_cancel(timer)    
...
reset_button = tk.Button(window, text="Reset", width=10, command=reset_funct, activebackground="green")
